...
export const AddCommentForm = ({params, setArticleInfo}) => {
    const [username, setUsername] = React.useState("");
    const [commentText, setCommentText] = React.useState("");

    const addComment = async () => {
       axios.post(`http://localhost:5001/api/article/${params.name}/add-comments`, {
            username: username,
            text: commentText
        }).then(response => {
    ...
    //     const result = await fetch(`http://localhost:5001/api/article/${params.name}/add-comments`, {
    //         method: "post",
